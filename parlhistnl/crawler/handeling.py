...
def __get_handeling_type(xml: ET.Element) -> str:
    ...
    return xml.findall("metadata[@name='DC.type'][@scheme='OVERHEIDop.HandelingTypen']")[0].get("content")


def __get_behandelde_kamerstukdossiers_and_kamerstukken(xml: ET.Element) -> dict[str, list[str]]:
    ...
    return {"behandelde_kamerstukken": kamerstukken, "behandelde_kamerstukdossiers": kamerstukdossiers}


def crawl_vergadering_subitem(vergadering: Vergadering, item: int, recrawl=False) -> Handeling:
    """Crawl a Handeling subitem of a vergadering"""

    try:
        existing_handeling = Handeling.objects.get(vergadering=vergadering, ondernummer=item)
    ...
    if recrawl or existing_handeling is None:
        ...
        try:
            ...
            logger.error("Couldn't crawl %s, assuming this vergadering subitem does not exist", html_url)
            raise CrawlerException("This vergadering subitem seems to not exist") from exc

        return create_or_update_handeling_from_raw_metadata_and_content(vergadering, item, meta_response.text, text_response.text, False)
    else:
        logger.info("Not recrawling existing handeling %s; updating metadata based on saved information", existing_handeling)
        return create_or_update_handeling_from_raw_metadata_and_content(
            vergadering, item, existing_handeling.raw_metadata_xml, existing_handeling.raw_html, True)
...
def create_or_update_handeling_from_raw_metadata_and_content(
        ...
        raw_html: str,
        raw_html_is_inner_html: bool) -> Handeling:
    ...
    if not raw_html_is_inner_html:
        ...
        if len(elems) > 1:
            logger.info("Got multiple matches where only one was expected %s %s", vergadering, item)
...
def crawl_uncrawled_behandelde_kamerstukken(handeling: Handeling) -> list[Kamerstuk]:
    ...
    uncrawled_kamerstukken: list[str] = handeling.data["uncrawled"]["behandelde_kamerstukken"]

    logger.info("Crawling the following uncrawled behandelde kamerstukken %s", uncrawled_kamerstukken)
    ...
    for uncrawled_kamerstuk in uncrawled_kamerstukken:
        ...
        try:
            ...
            logger.fatal("Received crawler exception when crawling kst-%s, %s, skipping (%s)", dossiernummer, ondernummer, exc)

    for removable_kamerstuk in removable_uncrawled_behandelde_kamerstukken:
        handeling.data["uncrawled"]["behandelde_kamerstukken"].remove(removable_kamerstuk)
...
def recrawl_behandelde_kamerstukken(handeling: Handeling) -> list[Kamerstuk]:
    ...
    for kamerstuk in kamerstukken:
        crawl_kamerstuk(
            kamerstuk.hoofddossier.dossiernummer,
            kamerstuk.ondernummer,
            update=True)

    return kamerstukken


def crawl_uncrawled_behandelde_kamerstukdossiers(handeling: Handeling) -> list[list[Kamerstuk]]:
    """Crawl behandelde kamerstukdossiers in a Handeling, and add the relevant relations in the database."""

    uncrawled_kamerstukdossiers: list[str] = handeling.data["uncrawled"]["behandelde_kamerstukdossiers"]

    logger.info("Crawling the following uncrawled behandelde kamerstukdossiers %s", uncrawled_kamerstukdossiers)
    ...
    for uncrawled_kamerstukdossier in uncrawled_kamerstukdossiers:
        try:
            ...
            handeling.behandelde_kamerstukdossiers.add(crawled_kamerstukken[0].hoofddossier)

            newly_added_kamerstukken.append(crawled_kamerstukken)
            removable_uncrawled_behandelde_kamerstukdossiers.append(uncrawled_kamerstukdossier)
        except CrawlerException as exc:
            logger.fatal("Received crawler exception while crawling kamerstukdossier %s, skipping (%s)", uncrawled_kamerstukdossier, exc)

    for removable_kamerstukdossier in removable_uncrawled_behandelde_kamerstukdossiers:
        handeling.data["uncrawled"]["behandelde_kamerstukdossiers"].remove(removable_kamerstukdossier)
...
def recrawl_behandelde_kamerstukdossiers(handeling: Handeling) -> list[list[Kamerstuk]]:
    ...
    kamerstukdossiers: QuerySet[KamerstukDossier] = handeling.behandelde_kamerstukdossiers.all()

    crawled_kamerstukken: list[list[Kamerstuk]] = []

    for kamerstukdossier in kamerstukdossiers:
        ksts = crawl_kamerstukdossier(kamerstukdossier.dossiernummer, update=True, ignore_failure=True)
